...
if ($result->num_rows == 0) {
    ...
    $create_cart = "INSERT INTO `tlipkart`.`cart` ( `customer` ) VALUES ('$user');";
    ...
    if($conn -> query($create_cart)){
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                $cart_id = $row['id'];
                $quantity = 1;
        $create_cart_item = "INSERT INTO `tlipkart`.`cart_item` ( `product`, `cart_id`, `quantity`) VALUES ('$id', '$cart_id', '$quantity');";
        if($conn -> query($create_cart_item)){
    ...
    $create_cart_item = "INSERT INTO `tlipkart`.`cart_item` ( `product`, `cart_id`, `quantity`) VALUES ('$id', '$cart_id', 1);";
    if($conn -> query($create_cart_item)){
     header("location:$prev_url");
